...
- **커버 이미지 프롬프트 전문** — 커버가 아직 없는 경우. 기웅이 이 파일에서 바로
  복사해 생성한다. 답변에 붙여넣지 않는다: 터미널 복사는 줄바꿈 유실과 선택 영역
  잘림이 잦고, 잘린 프롬프트는 스타일 토큰이 빠진 채 모델에 들어간다.
  전문은 이 파일에만 두고 `.images.md` 팩은 이 섹션을 가리킨다 (`IMAGE_GUIDE.md` §10)
