...
### 기웅에게 요청하는 방법 — 프롬프트는 한국어 리뷰 리포트 안에 둔다

**전달 위치는 `_reviews/YYYY-MM-DD-{slug}.ko.md`다.** 완성 프롬프트 전문을 그 파일의
전용 섹션에 넣고, 답변에서는 그 파일 경로와 섹션만 가리킨다.

이전 규칙은 "요청 답변 안에 프롬프트 전문을 붙인다"였다. **2026-08-21에 폐기했다** —
터미널 답변에서는 복사가 실패하는 경우가 잦고(줄바꿈 유실, 선택 영역 잘림, 스크롤백
버퍼 밖으로 밀림), 그렇게 잘린 프롬프트는 §6 스타일 토큰이 빠진 채 모델에 들어간다.
기웅은 어차피 리뷰 리포트를 열어서 검토하므로, 이미 여는 파일 안에 있는 편이 마찰이 없다.

**전문은 한 군데에만 둔다.** 리뷰 리포트가 전달본이고, 이미지 팩
(`_reviews/YYYY-MM-DD-{slug}.images.md`)은 소재 선정 근거·P6 점검·반입 검증 로그를
담되 프롬프트 전문을 다시 쓰지 않고 리뷰 리포트의 해당 섹션을 가리킨다. 같은 프롬프트를
두 파일에 복사해 두면 한쪽만 고쳐지는 순간 어느 쪽이 실제로 생성에 쓰인 것인지 알 수 없다.

리뷰 리포트의 이미지 섹션에 반드시 담을 4가지:
