...
| 생성 도구 | **gpt-image-2** (Codex CLI `$imagegen`) | **코드만** (Python/SVG). 이미지 모델 금지 |
...
## 10. Codex CLI 운용 (클래스 A 생성 경로)

### 반드시 `$imagegen`을 명시한다

Codex CLI는 **코딩 에이전트다.** 그냥 "커버 이미지 만들어줘"라고 하면 gpt-image-2를
부르지 않고 **Python/SVG로 도형을 그린다.** 2026-07-25에 레포에 들어간 3~4색
4KB 커버 12장이 정확히 그 결과물이다.

```bash
codex exec '$imagegen 아래 프롬프트로 이미지를 생성해서
/workspaces/ai-tech-blog/assets/img/posts/{slug}-cover-raw.png 에 저장해줘.

<프롬프트 팩의 완성 프롬프트 전체>'
```

`$imagegen`이 실제로 불렸는지는 **색상 수로 확인한다.** gpt-image 원본은 7,000~36,000색이
측정됐다. 3~4색이면 코드 렌더이며 폐기 대상이다:

```bash
python3 -c "
from PIL import Image; im=Image.open('경로-raw.png').convert('RGB')
n=len(im.getcolors(maxcolors=999999) or [])
print(im.size, n, 'colors', '→ OK' if n>5000 else '→ 코드 렌더. 재생성 필요')"
```

이 5,000 기준선은 **정규화 전 원본 PNG**에 적용한다. 크롭·리사이즈·JPEG 인코딩을
거치면 색상 수가 절반 가까이 줄어들기 때문에(실측 3,500~20,000), 최종 `.jpg`를 검사하는
훅 C11은 더 낮은 1,000을 쓴다. 두 숫자 모두 3~4색 코드 렌더와는 자릿수가 다르다.

생성 원본은 `~/.codex/generated_images/<session-id>/call_*.png`에도 남는다.

### 크기 — 1200×630은 직접 생성할 수 없다

gpt-image의 제약: 양변이 **16의 배수**, 최대 변 3840px 이하, 장변:단변 3:1 이내,
총 픽셀 655,360~8,294,400. 630은 16의 배수가 아니므로 요청 자체가 불가능하다.

**2048×1152 (16:9)로 생성한 뒤 1200×630으로 센터 크롭한다.** 상하에서 각각 약 38px,
전체의 6.7%만 잘린다. §7 COMPOSITION의 "상하 8% 안전 여백"이 이 크롭을 위한 것이다.

| 후보 크기 | 16의 배수 | 크롭 손실 | 판정 |
|---|---|---|---|
| **2048×1152** | ✓ | 상하 6.7% | **표준** |
| 1536×1024 | ✓ | 상하 21% | 손실 과다 |
| 1200×630 | ✗ (630) | — | 생성 불가 |
...
- 코드 렌더 산출물임 — `$imagegen` 미호출. 원본 PNG 5,000색 미만 또는 정규화된
