...
last_modified_at: 2026-09-21 17:00:00 +0900
...
- **Vision is now universal except one row**: every listed Anthropic, OpenAI, Google, and xAI model accepts image input, and so does `deepseek-flash` since it replaced the text-only v4-flash on September 14, 2026. DeepSeek v4-pro remains text-only.
...
Every figure on this page was collected on **2026-07-25** from first-party provider documentation only: [Anthropic](https://platform.claude.com/docs/en/about-claude/models/overview), [OpenAI](https://developers.openai.com/api/docs/models), [Google](https://ai.google.dev/gemini-api/docs/models/gemini-3.5-flash), [xAI](https://docs.x.ai/docs/models), [DeepSeek](https://api-docs.deepseek.com/quick_start/pricing), and [Mistral](https://docs.mistral.ai/getting-started/models/models_overview/). No aggregator, leaderboard, or third-party spec sheet was used as a source at any point.

The two DeepSeek rows are the exception to that date and carry their own: they were re-verified on **2026-09-21** after DeepSeek retired v4-flash and began serving the name from `deepseek-flash`. Every other row in every table below is the 2026-07-25 collection.
...
| deepseek-flash | DeepSeek | 1,000,000 | 384,000 | Yes | Not published |
...
The same logic applied to DeepSeek's text-only limitation until September 14, 2026, and the way it stopped applying is worth more than the limitation was. A pipeline touching screenshots, scanned documents or charts could not use DeepSeek at any price; the cheapest model that cannot do the task has an infinite effective cost, which is why an operational-envelope comparison belongs *before* a price comparison and not after it.

Then DeepSeek retired v4-flash and served the same name from a model that accepts images. No announcement of a capability launch, no new model id on the price list — the constraint simply stopped existing inside a row that had not changed its name. A reader who shortlisted on this table in July and did not re-check would still be excluding DeepSeek from image work today, on a limitation that now applies only to v4-pro. That is the failure mode an envelope comparison has that a price comparison does not: a price you quote stale is wrong by a percentage, and a capability you quote stale is wrong by a yes or no.
...
|---|---|---|
| Budget | deepseek-flash, Gemini 2.5 Flash-Lite, Claude Haiku 4.5 | High-volume classification, extraction, routing |
...
**Does the model accept your input at all?** If your pipeline touches images, DeepSeek v4-pro is out regardless of price, though `deepseek-flash` has accepted image input since September 14, 2026. If it touches video, audio, or PDFs natively, Gemini 3.5 Flash is the only model in this table that handles all three without a preprocessing step.
...
- **2026-09-21** — Out-of-cycle correction to the two DeepSeek rows, prompted by the weekly pricing collection rather than a scheduled refresh. DeepSeek retired v4-flash on 2026-09-14 and serves that name from `deepseek-flash`, which **accepts image input** where its predecessor did not. The capability table, the vision bullet, the envelope analysis and the shortlisting guidance all said DeepSeek was text-only; that is now true only of v4-pro. Context window (1,000,000) and max output (384,000) are unchanged on both rows and were re-verified. No other provider's rows were re-collected, so they remain the 2026-07-25 figures — see Methodology. The scheduled monthly refreshes of 2026-08-04 and 2026-09-01 were missed and are recorded as gaps rather than folded into this entry.
